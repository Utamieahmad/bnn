...
//        $baseUrl = URL::to('/');
    ...
    public function downloadPendataanTPPU(Request $request){
      $i = 1;
      $response = array();
      // dd($request->all());
      ...
      $kasusqry  = $kasusqry->where(function ($query) {
              $query->where('kategori', '=', 'tppu')->orWhere('kategori', '=', null);
          });
      ...
      // dd($kasusArray);
      $data = $kasusArray;
      $name = 'Export Data LKN TPPU '.Carbon::now()->format('Y-m-d H:i:s');
